...
public:
	...
	void backtrack(vector<NodeType>& sol, bool visited[], int time, bool poketmon_type_checkList[], int num_poketboll, int num_catch_poketmon, int num_must_go_pStop, int poketStop_time[], vector<int> &catch_order);
	bool promising(int num_catch_poketmon);
	void process_solution(vector<NodeType>& sol, int time, vector<int> &catch_order);
	void construct_candidates(vector<NodeType>& sol, bool visited[], vector<int>& cand, int num_poketball, bool poketmon_type_checkList[], int num_must_go_pStop);
	...
	void find_closest_poketmons(NodeType cur, vector<int>& closest_poketmon_node_index_list, bool visited[], bool poketmon_type_checkList[]);
