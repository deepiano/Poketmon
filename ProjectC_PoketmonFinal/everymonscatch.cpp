...
void EveryMonsCatch::backtrack(QVector<NodeType>& sol, bool visited[], int time, bool poketmon_type_checkList[], int num_poketball, int num_catch_poketmon, int num_must_go_pStop, int poketStop_time[], QVector<int> &catch_order)
{
    QVector<int> cand;
...
void EveryMonsCatch::construct_candidates(QVector<NodeType>& sol, bool visited[], QVector<int>& cand, int num_poketball, bool poketmon_type_checkList[], int num_must_go_pStop)
{
...
void EveryMonsCatch::make_all_route()
{
...
void EveryMonsCatch::find_closest_poketmons(NodeType cur, QVector<int>& closest_poketmon_node_index_list, bool visited[], bool poketmon_type_checkList[])
{
    for (int p_id = 1; p_id < 11; ++p_id)
    {
        ...
        if (poketmon_type_checkList[p_id] == false)
        {
            ...
            closest_poketmon_node_index_list.push_back(dest.index);//pid 10에서 에러
        }
    }
}
